  ...
  const createSort = () => {
    return sort.map((item, index) => (
      <div
        ...
        className={`animate__animated animate__fadeInLeft ${styles.sort} ${activeSort === item.id ? styles.selected : ''}`}
        onClick={() => {
    ...
    return work.map((item, index) => {
      ...
      return (
        <div
          key={item.id}
          style={{ animationDelay: `${index / 4}s` }}
          className={`animate__animated animate__fadeInDown ${styles.work} ${activeWork?.id === item.id ? styles.workSelected : ''}`}
          onClick={() => {
            setActiveWork(item);
          }}
        >
          {item.images.length > 0 && <img src={item.images[0].filePath} />}
          {item.images.length > 0 || item.videos.length > 0 && <video controls ><source src={item.videos[0].filePath} type="video/mp4" /></video>}
        </div>
      )
    });
  };
      ...
      <Fragment>
        {activeWork?.videos?.map(child => {
          return <div key={child.id}>
            <video controls ><source src={child.filePath} type="video/mp4" /></video>
            <p>{child.text}</p>
          </div>
        })}
        {activeWork?.images?.map(child => {
          return <div key={child.id}>
            <Image src={child.filePath} />
            <p>{child.text}</p>
          </div>
        })}
      </Fragment>
      ...
      <Layout>
        <div className={styles.container}>
          <div className={styles.leftContainer}>
            ...
            <Pagination current={current} total={total} pageSize={10} onChange={(e) => {
              setCurrent(e)
            }} />
          </div>
